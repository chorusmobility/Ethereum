...
contract DriverBehaviorDemo is Ownable{
    ...
    uint private supportedProtocolStatisticsAmount;
    
    uint private DriverScore;
    uint private AggressiveBreakingScore;
    uint private ExcessiveSpeedingScore;
    uint private HardBreakingScore;
    uint private PhoneUsageScore;

    uint private bid_limit; // Minimum limit for transaction based on contract
    
    function DriverBehaviorDemo() public{
        paid = false;
        owner = msg.sender;
        DriverScore = 0;
        supportedProtocolStatisticsAmount = 4;
        AggressiveBreakingScore = 0;
        ExcessiveSpeedingScore = 0;
        HardBreakingScore = 0;
        PhoneUsageScore = 0;
        tripStarted = false;
    }
    ...
    function canWithdraw() public constant returns (bool) {
        return ERC20(ERC20TokenAddress).balanceOf(ERC20TokenAddress) >= bid_limit;
    }
    ...
    //Update Supported limit for new one 
    function updateProtocolStatLimit(uint newLimit) public isOwner {
        supportedProtocolStatisticsAmount = newLimit;
    }
    
    function initDriverTrip(address driverAddress) public returns (uint256){
        if (ERC20(driverAddress).balanceOf(driverAddress) >= bid_limit) {
            setbeneficiary(driverAddress);
            //get money in case of starting the road game
            SafeERC20.safeTransferFrom(ERC20(ERC20TokenAddress), driverAddress, address(this), bid_limit);
    ...
    //Format: Main Stat: 0..100, Variable 4 kinds of stat: 0..10. Divider "+".
    function updateDriverStat(string arrayOfData) public returns (bool){
        slice memory s = toSlice(arrayOfData);
        slice memory delim = toSlice("+");
        string[] memory parts = new string[](count(s, delim));
        ...
        //Parsing via actual protocol
        DriverScore = stringToUint256(parts[0]);
        AggressiveBreakingScore = stringToUint256(parts[1]);
        ExcessiveSpeedingScore = stringToUint256(parts[2]);
        HardBreakingScore = stringToUint256(parts[3]);
        PhoneUsageScore = stringToUint256(parts[4]);
        
        tripStarted = false;
        
        withdraw();
        //send data into infura ipfs
        return true;
    }
    
    /* Preconditions
    Reward: [ 7.5..10.0]  Score * 100 (Tokens)
    Punishment: [0 .. 6.0) 100* (10 - Score) Tokens
    Refund: [6.0 .. 7.5) - nothing changed. */
    function withdraw() internal returns (bool) {
        require(canWithdraw());
        uint256 _amount = 0;
    
        //FUZZY LOGIC for Rewarding DB
        if (DriverScore > 75 && DriverScore <= 100){
            //SEND Reward
            //beneficiary.transfer();
            _amount = 10 * DriverScore;
            
        } else if (DriverScore > 60 && DriverScore <= 75) {
            //nothing changed 
            ...
            //PANISHMENT
            _amount = 10 * (100 - DriverScore);
        }
        
        SafeERC20.safeTransferFrom(ERC20(ERC20TokenAddress), address(this), beneficiary, _amount);
        return true;
    } 
     ...
     */
    function stringToUint256(string input) public pure returns (uint256 result) {
    ...
    //STRING solidity-stringutils @author Nick Johnson <arachnid@notdot.net>----
